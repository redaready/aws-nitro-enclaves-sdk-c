...
static void print_commands(int exit_code) {
    ...
    fprintf(stderr, "    sign: Sign a message with the given key id.\n");
    exit(exit_code);
}
...
static void s_usage_sign(int exit_code) {
    ...
    fprintf(stderr, "    --aws-session-token SESSION_TOKEN: Session token associated with the access key ID\n");
    fprintf(stderr, "    --key-id KEY_ID: key id\n");
    fprintf(stderr, "    --message message: The message digest to be signed\n");
    exit(exit_code);
}
...
static void s_parse_options(int argc, char **argv, const char *subcommand, struct app_ctx *ctx) {
    ...
    ctx->key_id = NULL;
    ctx->key_spec = AWS_KS_UNINITIALIZED;
    ...
    if (strncmp(subcommand, DECRYPT_CMD, MAX_SUB_COMMAND_LENGTH) == 0) {
        ...
        if (ctx->key_spec == AWS_KS_UNINITIALIZED) {
...
int main(int argc, char **argv) {
    ...
    if (strncmp(subcommand, DECRYPT_CMD, MAX_SUB_COMMAND_LENGTH) == 0) {
        ...
        fprintf(stdout, "SIGNATURE: %s\n", (const char *)signature_b64.buffer);
